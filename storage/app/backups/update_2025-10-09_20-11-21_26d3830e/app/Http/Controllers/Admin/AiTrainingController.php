...
class AiTrainingController extends Controller
{
    ...
    public function importFromUrl(Request $request)
    {
        ...
        try {
            ...
            // Real-time progress (polling) – USE TOKEN IF PROVIDED to avoid session locks/mismatch
            $token = (string) $request->input('progress_token');
            $token = is_string($token) ? trim($token) : '';
            if ($token && preg_match('/^[A-Za-z0-9._\-]{8,}$/', $token)) {
                $progressKey = 'url_train:' . $token;
            } else {
                // Fallback to per-user key (may not work without sessions)
            ...
            $result = $this->trainingService->trainFromUrl($validated['url'], [
                ...
                // Only crawl within the given URL path scope
                'scope_url' => $validated['url'],
                // Stop checker closure
    ...
    public function importStop(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['success' => true, 'message' => 'Import dayandırıldı']);
        } catch (\Throwable $e) {
            return response()->json(['success' => false, 'message' => 'Dayandırma alınmadı'], 500);
        }
    }
}
