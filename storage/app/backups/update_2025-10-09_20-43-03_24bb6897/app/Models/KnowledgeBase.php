        ...
        try {
            ...
            $urlTrainedResults = static::where('is_active', true)
                ...
                ->where(function ($q) use ($query, $expandedQuery) {
                    $q->where('title', 'LIKE', "%{$query}%")
                      ...
                      ->orWhereRaw("MATCH(title, content) AGAINST(? IN NATURAL LANGUAGE MODE)", [$expandedQuery]);
                })
            ...
            if ($remaining > 0) {
                $textTrainedResults = static::where('is_active', true)
                    ...
                    ->where(function ($q) use ($query, $expandedQuery) {
                        $q->whereRaw("MATCH(title, content) AGAINST(? IN NATURAL LANGUAGE MODE)", [$expandedQuery])
